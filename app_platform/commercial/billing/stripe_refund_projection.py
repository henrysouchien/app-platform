"""Fenced, order-tolerant persistence of authoritative Stripe Refunds."""
...
class StripeRefundProjectionError(RuntimeError):
    """Safe failure at the Refund projection authority boundary."""


@dataclass(frozen=True, slots=True)
class StripeRefundProjectionResult:
...
class PostgresStripeRefundProjector:
    """Persist normalized Refund authority without financial/access effects."""

    EVENT_TYPES = frozenset({"refund.created", "refund.updated", "refund.failed"})
        ...
        *,
    ...
    ) -> StripeRefundProjectionResult:
        if bool(getattr(self._connection, "autocommit", False)):
            raise RuntimeError("Stripe Refund projection requires a transaction")
        ...
        try:
            ...
            if webhook is None or webhook[1] not in self.EVENT_TYPES:
                raise StripeRefundProjectionError("Webhook identity is invalid")
            (environment, event_type, event_created_at, event_object_id,
             processing_state, active_lease, lease_unexpired) = webhook
            if event_type == "refund.failed" and snapshot.status != "failed":
                raise StripeRefundProjectionError(
                    "Failed Refund event is not authoritatively failed"
                )
            if environment != snapshot.environment or event_object_id != snapshot.external_object_id:
                raise StripeRefundProjectionError("Webhook Refund lineage is invalid")
            cursor.execute(
            ...
            if replay is not None:
                if (
                    ...
                    raise StripeRefundProjectionError(
                        "Refund replay does not match authority"
                    )
                return StripeRefundProjectionResult(
                    ...
                    disposition=str(replay[4]), durable_replayed=True,
                )
            if (
                ...
                raise StripeRefundProjectionError("Webhook lease is invalid")
            cursor.execute(
            ...
            cursor.execute(
                ...
                (snapshot.external_payment_intent_id, snapshot.external_charge_id,
                 environment, snapshot.external_payment_intent_id),
            )
            lineage = cursor.fetchone()
            if lineage is None or lineage[2] is None:
                raise StripeRefundProjectionError("Refund payment lineage is missing")
            ...
            if cursor.fetchone() is None:
                raise StripeRefundProjectionError("Refund account lineage is missing")
            cursor.execute(
            ...
            if cursor.fetchone() is None:
                raise StripeRefundProjectionError("Refund tenant lineage is missing")
            cursor.execute(
            ...
            if agreement is None:
                raise StripeRefundProjectionError("Refund agreement lineage is missing")
            ...
            cursor.execute(
                ...
                (snapshot.external_payment_intent_id, snapshot.external_charge_id,
                 environment, snapshot.external_payment_intent_id, account_id,
                 agreement_id, document_id),
            )
            if cursor.fetchone() is None:
                raise StripeRefundProjectionError("Refund payment lineage changed")
            cursor.execute(
            ...
            if authority is None:
                cursor.execute(
                    """INSERT INTO commercial_stripe_projection_objects (
                       ...
                       ) VALUES (%s, 'refund', %s, %s, %s, %s, 'current',
                                 %s, %s, %s, %s, %s, 1) RETURNING id, revision""",
                    (environment, snapshot.external_object_id, external_customer_id,
            ...
            if cursor.rowcount != 1:
                raise StripeRefundProjectionError("Webhook completion failed")
            return StripeRefundProjectionResult(
                projection_object_id=projection_object_id,
                projection_event_id=projection_event_id,
                object_revision=object_revision, agreement_version=agreement_version,
...
__all__ = [
    "PostgresStripeRefundProjector",
    "StripeRefundProjectionError",
    "StripeRefundProjectionResult",
]
